function changeMonths(numMonths) {
  ...
  d3.json(`/all-data/${$brand}/${numMonths}`, (error, data) => {
    ...
    $numMonths.html(`${data['months_past_sellthrough']}`);

    $lastUpdate.selectAll('em').remove();

    $lastUpdate.append('em').html(`Last update on ${data['last_update']} PST`);
    ...
    data['reorder_data'].forEach((item, index) => {
      ...
      $tbody
        .append('tr')
        .html(
          `<td class="text-left"><strong>${index +
            1}</strong></td><td class="text-left">${productCode}</td><td class="text-left">${description}</td><td class="text-right">${stockOnHand}</td><td class="text-right">${threshold}</td><td class="text-right">${indicator}</td><td class="text-right">${allocatedQty}</td><td class="text-right">${orderQty}</td><td class="text-right">${avgMonthlyUsage}</td><td class="text-right">${maxMonthlyUsage}</td><td class="text-right">${leadTimeDemand}</td><td class="text-right">${safetyStock}</td>`
        );
    });
  });
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const tHeadHeight = document.querySelector('[data-js=thead]').clientHeight;
  const tableHead = document.querySelector('[data-js=thead]');
  const tbodyWrapper = document.querySelector('.tbody-wrapper');

  // window.addEventListener('scroll', () => {
  //   // console.log('Scroll:', window.scrollY);
  //   // console.log('Header:', headerHeight);
  //   // console.log('Table Head:', tHeadHeight);

  //   try {
  //     if (window.scrollY > headerHeight) {
  //       tableHead.classList.add('sticky');
  //       tbodyWrapper.style['padding-top'] = `${tHeadHeight}px`;
  //     }
  //     if (window.scrollY < headerHeight) {
  //       tableHead.classList.remove('sticky');
  //       tbodyWrapper.style['padding-top'] = '0px';
  //     }
  //   } catch {}
  // });
});
